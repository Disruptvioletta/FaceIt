...
//  FaceIt
//
//  Created by Illarionova Violetta on 18/03/2017.
//  Copyright © 2017 Disruptvioletta LLC. All rights reserved.
//

import UIKit

class VCLoggingVC: UIViewController {
    ...
    private static func bumpInstanceCount(for className: String) -> Int { logGlobals.instanceCounts[className] = (logGlobals.instanceCounts[className] ?? 0) + 1
    ...
    private func logVCL(_ msg: String) {
        ...
        if instanceCount == nil {
            instanceCount = VCLoggingVC.bumpInstanceCount(for: className)
        }
        print("\(VCLoggingVC.logPrefix(for: className))(\(instanceCount!)) \(msg)")
    }
